...
1.0 正式版的核心目标很简单：打开软件后立刻进入一个有内容、有氛围、能直接开播的主页；播放音乐后切换到 Emily / 默认播放态视觉；需要探索歌单时，用右键唤起 3D 歌单架。它不是传统意义上的“歌单播放器”，而是把音乐播放、天气 mood、节奏分析、歌词空间和视觉镜头放进同一个桌面体验。
...
正式分发以 Release 资产中的 `Mineradio-1.0.0-Setup.exe` 为准，不建议直接下载 `win-unpacked` 目录作为正式分发包。安装包会创建桌面快捷方式；直接运行打包版 `Mineradio.exe` 时，应用也会在首次启动时补创建桌面快捷方式。
